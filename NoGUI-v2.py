...
for x in range(1, numVideos+1):
    ...
    files += [n]
    #l = input("What is the starting time? ")
    #startingtime += [l]
...
for j in files:
    ...
    while i < count:
        ...
        if i == 1:
            ...
            startingcountingsize = pixelcount * multiplicationthreshold
        if pixelcount >= startingcountingsize:
            ok = 1
        if ok == 1:
            tempframenumber += [i]
    ...
    del timeArray[0:tempdeletenumber]
