...
latest_weights = 'trainedNetworks/1790weights.h5'
...
def uploadVideos():
    filepath = tk.filedialog.askopenfilenames(filetypes=[("video format", ".avi")])
    i=0
...
def openNewWindow():
    try:
        ...
        def NextVideo(CurrentTotalVideoAmount, CurrentVideoData, df, newWindow, frame):
            ...
            line2.get_tk_widget().place(relx = 0.03, rely = 0.05, relwidth=0.6, relheight=0.6)
        ...
        def BackVideo(CurrentTotalVideoAmount, CurrentVideoData, df, newWindow, frame):
            #Calls function to get current videos number
            CurrentVideoNumber = GetCurrentNumber()
        ...
        for j in files:
            ...
            while success:
                ...
                #currentTime = image[470:500, 439:500]
        ...
        with open('embryo_results.csv', 'w', newline='') as csvfile:
            ...
            for j in filelist:
                ...
                thewriter.writerow({'Videos': j, 'Time (h)': finaltime[i], 'Initial Size (um^2)': initialsize[i], 
                                    ...
                                    'Average Growth Rate': growthsize[i], 'Avg Growth Rate Rank': 1})
                i += 1
        ...
        frame2.place(relx = 0.65, rely = 0.05, relwidth=0.30, relheight=0.3)
        label = tk.Label(frame2, text="Size Ranking")
        label.pack(expand=YES)
        tree1 = ttk.Treeview(frame2, columns=("Video", "Rank", "Final Size"))
        tree1.pack(expand=YES, fill=BOTH)
        tree1["columns"] = ("1", "2", "3")
        tree1['show'] = 'headings'
        tree1.column("1", minwidth=0, width=129)
        tree1.column("2", minwidth=0, width=129)
        tree1.column("3", minwidth=0, width=129)
        tree1.heading("1", text="Video")
        tree1.heading("2", text="Rank")
        tree1.heading("3", text="Final Size")
        number = 0
        ...
        frame3.place(relx = 0.65, rely = 0.40, relwidth=0.30, relheight=0.3)
        label = tk.Label(frame3, text="Average Growth Rate Ranking")
        label.pack(expand=YES)
        tree2 = ttk.Treeview(frame3, columns=("Video", "Rank", "Average Growth Rate"))
        tree2.pack(expand=YES, fill=BOTH)
        tree2["columns"] = ("1", "2", "3")
        tree2['show'] = 'headings'
        tree2.column("1", minwidth=0, width=129)
        tree2.column("2", minwidth=0, width=129)
        tree2.column("3", minwidth=0, width=129)
        tree2.heading("1", text="Video")
        tree2.heading("2", text="Rank")
        tree2.heading("3", text="Average Growth Rate")
        number = 0
        ...
        frame4.place(relx = 0.15, rely = 0.75, relwidth=0.3, relheight=0.118)
        label = tk.Label(frame4, text="Current Video")
        label.pack(expand=YES)
        tree3 = ttk.Treeview(frame4, columns=("Video", "Final Size", "Average Growth Rate"))
        tree3.pack(expand=YES, fill=BOTH)
        tree3["columns"] = ("1", "2", "3")
        tree3['show'] = 'headings'
        tree3.column("1", minwidth=0, width=129)
        tree3.column("2", minwidth=0, width=129)
        tree3.column("3", minwidth=0, width=129)
        tree3.heading("1", text="Video")
        tree3.heading("2", text="Final Size")
        tree3.heading("3", text="Average Growth Rate")
        ...
        RightButton.place(relx=0.45, rely=0.783, relwidth=0.02, relheight=0.085)
