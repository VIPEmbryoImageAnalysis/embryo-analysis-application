# LATEST TESTING SCRIPT
# DO ALL TESTING AND CHANGES IN THIS SCRIPT BEFORE MERGING WITH embryo-analysis FOR DEPLOYMENTS
...
latest_weights = os.path.join(base_path, 'trainedNetworks/1368weights.h5')
...
def openNewWindow():
    try:
        ...
        #define event: generate the comparison plot as well as display it on screen
        def genComparisonPlot(rawfinalTimeContainer, finalsize, videoNameContainer):
            plotWindow = tk.Toplevel(root)
            plotWindow.title("Comparison Plot")
            plotWindow.iconbitmap(icon)
            plotWindow.geometry("900x600")
            
            fig, ax = plt.subplots()
            ax.scatter(rawfinalTimeContainer,finalsize)
            
            for i, txt in enumerate(videoNameContainer):
                ax.annotate(txt, xy=(rawfinalTimeContainer[i], finalsize[i]),
                            xycoords='data', xytext=(-22, 8), textcoords='offset points')
                
            ax.set_xlabel('Time (h)',fontsize=15)
            ax.set_ylabel('Final Embryo Size ($um^2$)',fontsize=15)
            ax.set_title('Final Time + Size Comparison Plot', fontweight='bold', fontsize=30)
        ...
        videoNameContainer = []
        rawfinalTimeContainer = []
        # final size is already collected by variable finalsize
        # final time needed to be collected as raw version
        ...
        for j in files:
            ...
            while success:
                ...
                #store extracted time into an array
                timeArray.append(extractedTime)
                
                #store extracted time into another array for raw version
                rawtimeArray.append(extractedTime)
            ...
            rawfinalTimeContainer.append(rawtimeArray[-1])
        ...
        # Generate empty CSV and stores info of the files in it
        ...
        frame5.place(relx = 0.625, rely = 0.75, relwidth=0.325, relheight=0.2)
        ...
        yesRemoveButton1.place(relx=0.23, rely=0.1, relwidth=0.35, relheight=0.30)
        #style='Outline.TButton'
        yesRemoveButton2 = ttk.Button(frame5, text = 'Save CSV Results', style='info.TButton', command = lambda: saveResults(df, df3, filelist, tempPixelArray2))
        yesRemoveButton2.place(relx=0.6, rely=0.1, relwidth=0.40, relheight=0.30)
        #style='Outline.TButton'
        ...
        yesRemoveButton3.place(relx=0.23, rely=0.6, relwidth=0.35, relheight=0.30)
        #style='Outline.TButton'
        ...
        yesRemoveButton4.place(relx=0.6, rely=0.6, relwidth=0.40, relheight=0.30)
        #style='Outline.TButton'
        yesRemoveButton5 = ttk.Button(frame5, text = 'Comparison\n      Plot', style='info.TButton', command = lambda: genComparisonPlot(rawfinalTimeContainer, finalsize, videoNameContainer))
        yesRemoveButton5.place(relx=0.0, rely=0.1, relwidth=0.2, relheight=0.80)
